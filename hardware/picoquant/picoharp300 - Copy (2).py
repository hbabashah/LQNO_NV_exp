...
######################################################################################################
       ...
       #self.data_trace = np.zeros(actual_counts,dtype=np.int64)
            ...
            if marker_ch==0 or marker_ch==1 or marker_ch==2 or marker_ch==3 or marker_ch==4:
			    ...
			    if marker_ch=0 
					...
					if self.flag==1
                        ...
                        self.flag=1 #start
                if self.flag==1 and marker_ch=1
                    for i in range(round(self._record_length_ns/self._bin_width_ns)):
                        if (i-1)*self._bin_width_ns<=MytimeTag-timetag0<i*self._bin_width_ns:
                            self.data_trace1[i]=self.data_trace1[i]+1
        ...
        if self.finishTag==1: #finish
		    self.data_trace=self.data_trace1+self.data_trace
            self.finishTag=0
