...
#4:nactual:c_long(198) byref shode:<cparam 'P' (0000023DF89D1390)>
    ...
    def get_constraints(self):
        ...
        constraints['hardware_binwidth_list'] = [50e-9,0.1e-6,0.5e-6]
    ...
    def get_fresh_data_loop(self):
        ...
        if not self.meas_run:
            with self.threadlock:
                #self.unlock()
                print('unlock should be defined')
                self.stop_device()
...
######################################################################################################
                ...
                if marker_ch == 0 or marker_ch == 1 or marker_ch == 2 or marker_ch == 3 or marker_ch == 4:
                    if marker_ch == 0:
                        ...
                        if self.startflag == 1:
                            ...
                            if self.finishtime0:
                                self.finishtime=max(self.finishtime0,self.timetag0*Myresolution)
                            self.finishtime0=time_tag*Myresolution-self.timetag0
                        else:
        ...
        if self.finishtime:
            ...
            print(self.data_trace_helper[0:round(float(self.finishtime)/(self._bin_width_ns*1e3))])
            print(self.data_trace)
            self.data_trace=self.data_trace_helper[0:round(float(self.finishtime)/(self._bin_width_ns*1e3))]
            print(self.data_trace)
