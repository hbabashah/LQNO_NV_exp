...
class PicoHarp300(Base, SlowCounterInterface, FastCounterInterface):
    ...
    def tttr_read_fifo(self):#, num_counts):
        ...
        #num_counts=10
        print('inside readout1')
        print(num_counts)
        #buffer = np.zeros((num_counts,), dtype=np.uint32) #previous buffer
        #my defined buffer
        buffer = (ctypes.c_uint * self.TTREADMAX)()
        print('inside readout2')
        #print(buffer.ctypes.data)
        actual_num_counts = ctypes.c_int32()
        print('actual count=')
        print(actual_num_counts)
        print('afsgd')
        print(ctypes.byref(actual_num_counts))
        ...
        print('self._deviceID')
        print(self._deviceID)
        print('L768TTTRREAD/Picoharp300')
...
#4:nactual:c_long(198) byref shode:<cparam 'P' (0000023DF89D1390)>
        print('inside readout4')
        print('781/buffer[1]=')
        print(buffer[1])
        print('783/actual_num_counts.value')

        print(actual_num_counts.value)
        print('781/actual_num_counts.value printed')
    ...
    def get_constraints(self):
        ...
        constraints['hardware_binwidth_list'] = [4e-12,8e-12,32e-12,64e-12,128e-12,256e-12,512e-12,1024e-12,2048e-12,4096e-12,8192e-12,16384e-12]
    ...
    def configure(self, bin_width_ns, record_length_ns, number_of_gates = 0):
        ...
        self.data_trace = np.zeros(int(round(self._record_length_ns/self._bin_width_ns+1)), dtype=np.int64 ) #modified
    ...
    def start_measure(self):
        ...
        self.meas_run = True
        print('start measure record length in ns is')
...
######################################################################################################
######################################################################################################
        self.data_trace[self.count] = actual_counts        # hossein edited this part
        self.count += 1
        print('picoharp1359/self.count=')
        print(self.count)
...
######################################################################################################
        ...
        k=0
		Myresolution=4e-12 #this is picoHarp resolution which is needed to be modified--> binning
        self.data_trace = np.zeros(actual_counts,dtype=np.int64)
        for entry in arr_data[0:actual_counts-1]:
            ...
            T2time = entry & (2**32 -1 - 2**(32-1) - 2**(32-2) - 2**(32-3) - 2**(32-4)) 
            time_tag = T2time+ofltime			
            if marker_ch==0 or marker_ch==1 or marker_ch==2 or marker_ch==3 or marker_ch==4:
                MytimeTag=time_tag*Myresolution #it is in second
                for i in range(round(self._record_length_ns/self._bin_width_ns)):
			        if (i-1)*self._bin_width_ns<=MytimeTag<i*self._bin_width_ns:
						self.data_trace[i]=self.data_trace[i]+1
        ...
        #self.data_trace=[142607140,142607180,u142607220,142607260]
       #00 print(arr_data)np.random.randint(2, size=10)
	   #entry & (2**(32-2)  + 2**(32-3) + 2**(32-4))
        self.data_trace=np.int64(self.get_data_testfile()) # Hossein added this for test
        print('picoharp/s')
        print(self.data_trace)
        #self.data_trace=np.array(arr_data)
        #self.data_trace=np.array(arr_data[0:actual_counts-1],dtype=np.int64)
        #self.data_trace=np.array([1,2,3,4,5,6])
        #self.data_trace=np.array(np.random.randint(2,size=actual_counts),dtype=np.int64)		
        print('entry')
        print(entry)
        print('1296Picoharp')
        print(marker_ch)
