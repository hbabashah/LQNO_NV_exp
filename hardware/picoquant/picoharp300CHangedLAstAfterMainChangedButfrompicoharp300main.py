...
class PicoHarp300(Base, SlowCounterInterface, FastCounterInterface):
    ...
    def start(self, acq_time):
        ...
        if not(self.ACQTMIN <= acq_time <= self.ACQTMAX):
            ...
            print('acq_time=')
            print(acq_time)
            self.check(self._dll.PH_StartMeas(self._deviceID, ctypes.c_int(int(acq_time)))) #here there were a problem with acq_time which was 1 but it should be e.g around 1000
    ...
    def tttr_read_fifo(self):#, num_counts):
        ...
        #buffer = np.zeros((num_counts,), dtype=np.cint32)
        print('Picoharp740/buffer changed to ctypes intead of numpy array')
        actual_num_counts = ctypes.c_int()
        print(self._deviceID)
        self.check(self._dll.PH_ReadFiFo(self._deviceID, ctypes.byref(buffer),
                                         num_counts, ctypes.byref(actual_num_counts)))
        countRate0 = ctypes.c_int()

        self.check(self._dll.PH_GetCountRate(self._deviceID, ctypes.c_int(0),
                                         ctypes.byref(countRate0)))
        print('CountRate0=')
        print(countRate0.value)
        print('buffer[0]=')
        print(buffer[0])
        print(buffer[1])
        print(buffer[2])
    ...
    def get_constraints(self):
        ...
        constraints['hardware_binwidth_list'] = [4e-12,8e-12,16e-12,32e-12,64e-12,128e-12,256e-12] #This value was chose by Hoda and Hossein it is in nanoseconds
    ...
    def configure(self, bin_width_ns, record_length_ns, number_of_gates = 0):
        ...
        self.data_trace = np.zeros(int(number_of_gates+1), dtype=np.int64 ) #modified
    ...
    def start_measure(self):
        ...
        """
        print('Picoharp1173/recent changed')
       # self.lock()
